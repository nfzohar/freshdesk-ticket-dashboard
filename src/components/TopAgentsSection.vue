<template>
  <div class="w-full">
    <h1 class="text-xl font-bold mb-1" v-text="'Top agents'" />
    ...
    >
      <a-top-agent v-for="(agent, t) in topAgents" :key="t" :position="t" :agent="agent" />
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    calculateAgentStatistics() {
      ...
      this.allAgents.forEach((agent) => {
        ...
        this.topAgents.push(anAgent)
      })

      this.topAgents = this.topAgents.sort((a1, a2) => {
